...
export default function Home({
  ...
  return (
    ...
    >
      <Events
        locale={locale}
        titleEvent={mainPageFields?.titleEvent}
        posts={events}
      />
      <EventsMobile
        locale={locale}
      ...
      {services?.nodes.length > 0 && (
        <Element name="#Services" className="element">
          <Services
            locale={locale}
            titleServices={mainPageFields?.titleServices}
            posts={services.nodes}
            pageInfo={services.pageInfo}
          />
        </Element>
      )}

      {popularProjectsData?.projects?.length > 0 && (
        <ProjectsWrapper locale={locale} posts={popularProjectsData} />
      )}
      {news.nodes.length > 0 && (
        <LastNews
          title={NewsLsi.lastNews}
          locale={locale}
          titleNews={mainPageFields?.titleNews}
