...
export default function EventCalendar({
  ...
  const timeFormatted = new Date(
    time.hoursEvents?.hoursEvents.replace(/-/g, "/")
  );
  ...
  const selectedDay = (value) => {
    ...
    router.push(
      ...
      `/calendar/date/${formatDate(value).substring(0, 10)}`,
      { shallow: false }
    );
  };
  ...
  return (
    <MainLayout
      databaseId={time.databaseId}
      contacts={contacts}
      menu={parsedMenu}
      routerLinkTitle={time.title}
    >
      <Container>
        <Header>
          <Title>
                ...
                <Calendar
                  ...
                  tileDisabled={({ date, view }) =>
                    ...
                    allDates.every(
                      (disabledDate) =>
                        formatDate(date) !==
                        disabledDate.dateGmt.toString().substring(0, 10)
                    )
                  }
                />
              </CalendarContainer>
...
export async function getStaticProps({ params, locale }) {
  ...
  const allDates = futureDate?.data?.events?.nodes?.concat(
    publishDate?.data?.events?.nodes
  );
...
export const getStaticPaths = async ({ locales }) => {
  ...
  for (const locale of locales) {
    paths = [
      ...paths,
      ...data.times?.nodes.map((el) => ({
        params: { currentHourId: el.databaseId.toString() },
        locale,
      })),
    ];
  }
