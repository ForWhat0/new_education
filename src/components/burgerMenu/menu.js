    ...
    <StyledMenu open={menuBurgerIsOpen} ref={ulRef}>
        ...
        <Header>
          <Link href={"/"}>
            <a onClick={() => handlerCloseMenu()} style={{ zIndex: "6" }}>
              <LogoImg height="70px" width="70px" src="/headerLogo.svg" />
            </a>
          </Link>
          <Burger />
        </Header>
      ...
      <Ul>
          ...
          button.children.length > 0 ? (
              ...
              el.path.charAt(0) === "#" ? (
                ...
                <Link key={i + el.title} href={ParcUri(el.path)}>
                  <Li onClick={() => handlerCloseMenu()}>
                    <ALink
                      activeLink={
                        router.pathname === ParcUri(el.path) && activeLink
                      }
                    >
                      {el.title}
                    </ALink>
            ...
            <Link key={i + button.title} href={ParcUri(button.path)}>
              <Li onClick={() => handlerCloseMenu()}>
                <ALink
                  activeLink={
                    router.pathname === ParcUri(button.path) && activeLink
                  }
                >
                  {button.title}
                </ALink>
        ...
        <ChangeLanguageSelector
          navMain={true}
          globeDarkIcon="/glassIconDark.svg"
        />
