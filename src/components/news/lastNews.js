...
export default function LastNews({
  ...
  const { newsForMobileSliderReducerPageInfo } = useSelector(
    (state) => state.news
  );
  const dispatch = useDispatch();
  const { visuallyImpairedModeWhiteTheme } = useSelector((state) => state.app);
  const news = newsReducer?.data?.news?.nodes
    ? newsReducer.data.news.nodes
    : posts;
  const newsForMobile = newsForMobileSliderReducer
    ? newsForMobileSliderReducer
    : posts;

  const hasNextPageForMobile = newsForMobileSliderReducer
    ? newsForMobileSliderReducerPageInfo
    : pageInfo.offsetPagination.hasMore;
  const hasNextPage = newsReducer?.data?.news?.pageInfo?.offsetPagination
    ? newsReducer.data.news.pageInfo.offsetPagination.hasMore
    : pageInfo.offsetPagination.hasMore;
  const hasPreviousPage = newsReducer?.data?.news?.pageInfo?.offsetPagination
    ? newsReducer.data.news.pageInfo.offsetPagination.hasPrevious
  ...
  const nextNewsForMobile = () => {
    ...
    dispatch(
      actionGetNextNewsForMobile(
        !newsForMobileSliderReducer ? offsetMobile : offsetMobile + 3,
        newsForMobile,
        locale
      )
    );
      ...
      <Back padding={padding} background={background}>
        <Container>
          ...
          <ScrollBarStyledContainer>
            <ScrollBarStyled ref={inputRef} onScroll={checkScroll}>
          ...
          <ButtonContainer display={buttonDisplay}>
            <Link href={"/news"}>
              <a>
                <StyledButton text={NewsLsi.moreNews[locale]} />
              </a>
            </Link>
          </ButtonContainer>
        </Container>
