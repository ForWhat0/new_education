...
export const ChangeLanguageSelector = ({ theme, globeDarkIcon }) => {
  ...
  const { languages } = app;
  const globeIcon = globeDarkIcon
    ? "/changeLanguageIconDark.svg"
    : "changeLanguageIcon.svg";
  const color = theme ? "#3F3F3F" : globeDarkIcon ? "#3F3F3F" : "white";
  const visuallyModeIcon = theme
    ? "/changeLanguageVisyalMode.svg"
    : globeDarkIcon
    ? "/changeLanguageVisyalMode.svg"
    : "/changeLanguageVisyalModeWhite.svg";
  ...
  return (
    <Select
      width={fontSize === "normal" ? "163px" : "200px"}
